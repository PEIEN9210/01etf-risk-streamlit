...
st.title("📊 台灣 ETF 多準則資產排序框架 (僅供參考，不負投資風險:)")
...
st.sidebar.write(
    f"HotIndex 權重 (內生 α): {ALPHA_MODEL:.2f} | 個人化分數權重: {1 - ALPHA_MODEL:.2f}"
)
st.sidebar.write(
    f"(手動滑桿 α 參考值: {ALPHA_SLIDER:.2f}, 不影響排序)"
)
...
radar_closed = pd.concat([
    radar,
    radar.groupby("ETF").apply(lambda d: d.iloc[[0]]).reset_index(drop=True)
], ignore_index=True)
